...
import com.yogurt.domain.staff.infra.AdminStaffRepository;
...
public class AdminStaffServiceImpl implements AdminStaffService {
    ...
    public Staff getById(Long id) {
        return adminStaffRepository.findById(id).orElseThrow(() -> new YogurtDataNotExistsException("존재 하지 않는 직원입니다."));
    }

    @Transactional
    public Staff getByUser(User user) {
        return adminStaffRepository.findByUser(user).orElseThrow(() -> new YogurtDataNotExistsException("존재 하지 않는 직원입니다."));
    }

    @Transactional
    public List<Staff> getAllWithFilter(Pageable pageable, Boolean isDeleted) {
        return adminStaffRepository.getAllWithFilter(pageable, isDeleted);
    }
    ...
    public void resetPassword(long userId) {
        ...
        String randomPassword = StringUtils.getRandomPassword();
        commonUserService.changePassword(userId, randomPassword);
